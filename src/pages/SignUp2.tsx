...
export default function SignUp2() {
  ...
  const savePlaylist = async (userEmail: string, url: string, userName: string) => {
    const response = await fetch("http://localhost:8000/save-playlist", { //http://localhost:8000
      method: "POST",
      headers: {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (playlistUrl) {
        ...
        // Call process_all via the /run-cron endpoint
        const response = await fetch("http://localhost:8000/run-cron"); //http://localhost:8000
        const result = await response.json();
        console.log("Process All Result:", result);
      }
