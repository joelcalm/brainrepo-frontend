...
export default function SignUp2() {
  ...
  // The playlistUrl is passed from Index.tsx or Login2.tsx if the user
  // first entered the playlist on the homepage
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (playlistUrl) {
        await savePlaylist(currentUser.email ?? "", playlistUrl, name);
      }
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const currentUser = userCred.user;
      const userName = currentUser.displayName || name;
